...
app.post("/getClassifiedResults", async function (req, res) {
  ...
  for (let i = 0; i < googleResults.items.length; i++) {
    ...
    try {
      let openData = `Given the question "${query}", please identify if the following source supports either "${categories[0]}" or "${categories[1]}" and give the explanation (FOLLOW THE GIVEN FORMAT:  1) Answer: "${categories[0]}" or "${categories[1]}" 2) Explanation: ___ ): "${article}"`;
...
async function identifyQuestionsPossibleAnswers(question) {
  ...
  let proResult = response
    .split("2")[0]
    .replace("1. ", "")
    .replaceAll(/\s/g, "");
  let conResult = response
    .split("2")[1]
    .replace(". ", "")
    .replaceAll(/\s/g, "");
  console.log(
    "Responses can be classified in the following categories: " +
      proResult +
      " / " +
      conResult
  );
